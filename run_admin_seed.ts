...
async function seedWithAdmin() {
  try {
    ...
    console.log("Admin Seeding to Project ID:", config.projectId);
    console.log("Admin Seeding to Database ID:", config.firestoreDatabaseId);

    // Set environment variable so the Admin SDK automatically targets the named database
    if (config.firestoreDatabaseId) {
      process.env.FIRESTORE_DB = config.firestoreDatabaseId;
    }
    ...
    const initialContent = [
      {
        ...
        excerpt_uz: "O'zbekiston iqtisodiyotini modernizatsiya qilish yo'dilidagi asosiy qadamlar va kutilayotgan natijalar haqida batafsil tahlil.",
    ...
    console.log("Adding and seeding standard documents with Admin SDK root privileges...");
    for (const item of initialContent) {
      console.log(`Writing Article: ${item.id} - ${item.title_en}`);
      await db.collection("content").doc(item.id).set({
        ...item,
        created_at: admin.firestore.FieldValue.serverTimestamp()
      }, { merge: true });
    }
    console.log("SUCCESSFULLY COMPLETED SEEDING OF ALL 10 ARTICLES!");
    process.exit(0);
  } catch (err: any) {
